...
- 应用：ApplicationService、ApplicationConfigurationService和ApplicationCredentialService。任务：Tasks、JobReadService；IndexAccountingService拥有索引计数和模型调用台账；IndexGenerationService管理任务代际切换，IndexMaintenanceService负责完整核对和重建入口。用量管理：UsageAdministrationService；QueryReservationService拥有查询预占和并发门禁，RetrievalAccounting记录检索消耗；RetrievalService编排检索。
