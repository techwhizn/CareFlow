...
部署脚本还会生成 `DEFAULT_ACCESS_TOKEN`。它可作为固定的临时所有者登录凭证；通过部署管理员“开通另一企业”创建租户时，也会优先将其签发给新企业所有者。首个企业初始化接口仍返回一次性所有者凭证。该默认凭证长度至少 32 个字符，只保存在部署机 `.env`；使用它登录时页面会持续提示风险，应通过成员管理签发个人凭证并撤销默认凭证，再按需更新部署配置。

## 安全检查

不要把密钥放在前端、源码、命令历史、截图或 Issue 中。启动后通过健康检查、模型连接检测和一条合成资料验证配置；缺少关键配置应修复依赖后重试，不使用模拟服务掩盖失败。
在 Docker Compose 中，如果 Embedding 或重排服务运行在宿主机，模型档案可以填写 `http://localhost:8092/v1`。Worker 会依据 Compose 注入的 `MODEL_HOST_ALIAS` 将运行时请求转发到 `host.docker.internal`，不会改变已发布索引的模型身份。若模型服务运行在独立容器，请直接填写该服务名和端口，并在模型地址许可中登记。
