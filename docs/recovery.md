...
备份清单还保存RabbitMQ的hostname、节点名和长名称模式。恢复目录生成`recovery-broker.compose.json`，之后创建恢复容器时必须作为最后一个Compose覆盖文件传入，例如`-f /secure/careflow-recovery-20260911/recovery-broker.compose.json`。不能仅更换项目名后直接启动：RabbitMQ数据目录依赖节点名，改变名称可能创建空队列。该文件不包含凭证；它固定备份节点身份，适用于独立隔离恢复网络，不用于把恢复节点加入原集群。

缺少节点身份的旧清单会在创建卷之前拒绝恢复。原部署仍存在时重新捕获完整备份；无法确认身份时保持访问关闭，不猜测目录、不直接修改已签名清单。自定义Mnesia目录、环境脚本或节点名与容器hostname不一致的部署，需要单独恢复方案。新节点名不会修复已误启动为空队列的实例，必须回到新空目标重做恢复并核对持久队列/绑定/消息。

`STORAGE_RESTORED_ACCESS_CLOSED` 仅表示存储提取完成，不代表产品可用。数据库必须先在无公共端口的隔离网络启动，使用相同兼容镜像核对数据、迁移、原文件摘要及模型身份。在开放任何API、重建索引或恢复Worker前，仍须重放备份之后最新的删除、权限收窄、成员禁用、应用绑定变化和凭证撤销。无法确认最新安全状态时保持全部业务入口关闭。日志前滚见下文；完整冷恢复验收见[实际记录](reports/v1-46-recovery.md)，工具不提供自动解除该门禁的命令。
